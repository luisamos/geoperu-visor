...
function setZoomMapa() {
    let _alto = document.querySelector('#map').offsetHeight;
...
const optionsFuse = {
    ...
    limit: 10,
    tokenize: true,
    keys: ['nom_dpto', 'nom_prov', 'nom_dist']
};
...
$filtroUbigeo.addEventListener('input', function () {
    ...
    searchUbigeo.forEach(function (el) {
        if (el.cod_dpto.toString() !== '00')
            $listUbigeo.insertAdjacentHTML('beforeend', `<li class="li_lnkUbigeo">
                <span onclick="setLimitLocalidad('${el.cod_dpto.toString()}', '${el.nom_dpto}')">${el.nom_dpto}</span> > 
                <span onclick="setLimitLocalidad('${el.cod_prov.toString()}', '${el.nom_prov}')">${el.nom_prov}</span> > 
                <span onclick="setLimitLocalidad('${el.cod_dist.toString()}', '${el.nom_dist}')">${el.nom_dist}</span>
            </li>`);
        else
            $listUbigeo.insertAdjacentHTML('beforeend', `<li class="li_lnkUbigeo">
...
$ubigeoImage.addEventListener('click', function () {
    $filtroUbigeo.value = '';
    resetFiltroUbigeoImage();
    $filtroUbigeo.focus({ preventScroll: false });
});
...
async function setLimitLocalidad(_ubigeo, _localidad) {
...
function setOpacityDistinct(_ubigeo) {
    ...
    var ubigeoContextPeru = L.nonTiledLayer.wms(cqlURL,
        {
            maxZoom: 20, minZoom: 0, opacity: 1.0, layers: strLayer, zIndex: 1,
    ...
    var ubigeoContextSudamerica = L.nonTiledLayer.wms(`${DOMINIO_ESPACIAL}/geoserver/geoperu/wms?service=WMS&`,
        {
            maxZoom: 20, minZoom: 0, opacity: 1.0, layers: 'peru_sudamerica_mascara_',
...
function setOpacityDistinctAmbito(_ambito) {
    ...
    let ubigeoContextPeru = L.nonTiledLayer.wms(cqlURL,
        {
            maxZoom: 20, minZoom: 0, opacity: 1.0, layers: layerContextPeru, zIndex: 1,
            format: 'image/png8', transparent: true, pane: 'ubigeoPane', useCanvas: false
        });
    ...
    var ubigeoContextSudamericaEspecial = L.nonTiledLayer.wms(`${DOMINIO_ESPACIAL}/geoserver/geoperu/wms?service=WMS&`,
        {
            maxZoom: 20, minZoom: 0, opacity: 1.0, layers: 'binacional_pe_sudamerica_mascara_',
            format: 'image/png8', transparent: true, pane: 'ubigeoPane', zIndex: 1, useCanvas: false
        });
...
async function showInformacionIdLayer(_layerXML) {
    ...
    let respJson = await ASYNC_FETCH_JSON(URL_GET_INFO_LAYER + _layerXML);
    ...
    $spanDefinition.innerHTML = DOMPurify.sanitize(oInformacion.definicion);
    ...
    if (respGetFilesJson.filter(i => i.item === 1)[0] !== undefined) {
        $lnkInfoFile_1.classList.remove('displayNone');
        $lnkInfoFile_1.innerHTML = respGetFilesJson.filter(i => i.item === 1)[0].nombre
        $lnkInfoFile_1.href = respGetFilesJson.filter(i => i.item === 1)[0].url_file
    }
    else
        $lnkInfoFile_1.classList.add('displayNone');
    ...
    if (respGetFilesJson.filter(i => i.item === 2)[0] !== undefined) {
        $lnkInfoFile_2.classList.remove('displayNone');
        $lnkInfoFile_2.innerHTML = respGetFilesJson.filter(i => i.item === 2)[0].nombre
        $lnkInfoFile_2.href = respGetFilesJson.filter(i => i.item === 2)[0].url_file
    }
    else
        $lnkInfoFile_2.classList.add('displayNone');
    ...
    if (respGetFilesJson.filter(i => i.item === 3)[0] !== undefined) {
        $lnkInfoFile_3.classList.remove('displayNone');
        $lnkInfoFile_3.innerHTML = respGetFilesJson.filter(i => i.item === 3)[0].nombre
        $lnkInfoFile_3.href = respGetFilesJson.filter(i => i.item === 3)[0].url_file
    }
    else
        $lnkInfoFile_3.classList.add('displayNone');
...
//Función para enviar mensajes de aleta.
...
async function readWMS(_url) {
    ...
    try {
        ...
        let respURL = await fetch(urlProxy, { method: 'GET' })
        ...
        if (strAllLayer.length > 0) {
            ...
            for (var i = 0; i < arrayLayers.length; i++) {
                ...
                Object.defineProperty(allWMSExternal, identificador, {
                    value: L.tileLayer.betterWms(`${urlCurrentService}`, { layers: idCurrentCapaWMS, format: 'image/png', transparent: true, version: '1.0', crs: L.CRS.EPSG4326, opacity: 1.0, pane: 'defaultPane', nombre: nameCurrentCapaWMS, urlLegend: urlLegendCapaWMS })
                });
        ...
        if (strAllLayer.length > 0) {
            ...
            for (var i = 0; i < arrayLayers.length; i++) {
                ...
                Object.defineProperty(allWFSExternal, idCurrentCapaWFS, {
                    value: new L.WFS({
                        url: _url.indexOf('https') ? `${_url.substr(0, _url.indexOf('?') + 1)}` : `${URL_PROXY}${_url.substr(0, _url.indexOf('?') + 1)}`,
...
const $optionWeight = document.getElementsByName('optionWeight');
