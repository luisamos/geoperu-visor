...
var optionsBase = { layers: 'base_elevaciones', format: 'image/png8', maxZoom: 20, transparent: true, /*crs: L.CRS.EPSG4326, */pane: 'defaultPane', opacity: 1.0, zIndex: 1, tiled: true }
...
async function addNodesLayers() {
    ...
    mLayer.forEach(layer => {
        ...
        if (allCapas[layer.capaxml] === undefined) {
            ...
            if (layer.idfuente === 2 && layer.alias.trim().length === 0)
                return;
            let urlEspacial = '';
            ...
            switch (layer.idfuente) {
                case 1: urlEspacial = URL_SERVICE_GEOSERVER + 'geoperu/wms?service=WMS&';
                    break;
                case 2: urlEspacial = layer.url_fuente.substr(0, layer.url_fuente.indexOf('?') + 1);
                    break;
                case 3:
                case 4: urlEspacial = URL_SERVICE_GEOSERVER + 'subsistemas/wms?service=WMS&';
                    break;
                case 5: urlEspacial = layer.url_fuente.substr(0, layer.url_fuente.indexOf('?') + 1);
                    break;
                case 6:
        ...
        //     `<li>
        //         <section data-code="${(layer.idfuente === 2 || layer.idfuente === 5) ? layer.alias : layer.capaxml}">
        //             <img src="./images/nav/nav-accordion/info.png" alt="Ícono de información" onclick="showInformacion('${((layer.idfuente === 2 && layer.nombre_capa !== undefined) ? layer.nombre_capa : layer.capaxml)}');">
        ...
        $uParent.insertAdjacentHTML('beforeend',
            `<li>
                <section data-code="${(layer.idfuente === 2 || layer.idfuente === 5) ? layer.alias : layer.capaxml}" data-subsistema="${layer.idsubsistema}">
                    <img src="./images/nav/nav-accordion/info.png" alt="Ícono de información" onclick="showInformacionIdLayer('${layer.id}');">
                    <input class="checkbox__class" type="checkbox" name="checkbox" id="chkLayer_${layer.id}" data-source="${layer.idfuente}" data-xml="${layer.capaxml}" data-name="${layer.capa}">
                    <label for="chkLayer_${layer.id}" id="label_${layer.id}" title="${layer.tooltip_layer}">${layer.capa}</label>
                    <input id="rngLayer_${layer.id}" name="range" type="range" min="0" max="10" value="10" disabled  />
                </section>
...
function addLayerToObjectMap(_layer, _urlService, _externalFile = false) {
    if (!_externalFile) {
        if (_layer.tilerequired) {
            ...
            // ERROR DE CORB
            // Object.defineProperty(allCapas, _layer.capaxml, { 
            //     value: L.tileLayer(`${URL_PROXY}${_urlService}`, {
            //         pane: 'defaultPane',
            //         opacity: 1.0,
            //         format: 'image/png',
            //         layers: _layer.capaxml,  
            ...
            if (_layer.idfuente === 2) {
                ...
                Object.defineProperty(allCapas, _layer.alias, {
                    value: L.nonTiledLayer.wms(urlCurrentWMS, {
                        opacity: 1.0, format: 'image/png', maxZoom: 20, transparent: true, pane: 'defaultPane', zIndex: _layer.zindex,
                ...
                Object.defineProperty(allCapas, _layer.capaxml, {
                    ...
                    value: L.nonTiledLayer.wms(`${_urlService}`, {
                        ...
                        opacity: 1.0,
                        format: 'image/png8', 
                        layers: _layer.idfuente == 1 || _layer.idfuente == 2 ? _layer.capaxml : _layer.url_fuente.replace('.zip', ''),
                        maxZoom: 20,
                        transparent: true,  
                        zIndex: _layer.zindex,
                        idfuente: _layer.idfuente,
...
function addLayerDefault(_layersDefault = LAYERS_DEFAULT) {
    let mDefault = _layersDefault.split('|');
...
async function loadLayerDefaultAsync(_layersLoad, _setBBox = false) {
    ...
    if (codSubsistemaCurrent === 'M1' && _layersLoad === 'peru_mr_andes_') {
    ...
    if (_setBBox) {
        ...
        if (itemBBox.length > 0) {
            ...
            if (itemBBox[0].name === 'peru_vraem_' || itemBBox[0].name === 'peru_corredor_minero_' ||
                ...
                itemBBox[0].name === 'peru_mr_amazonica_' || itemBBox[0].name === 'binacional_pe_sudamerica_mascara_') setOpacityDistinctAmbito(itemBBox[0].name);
        }
    }
}
...
function toggleStateCheckLayer(_element) {
...
function addLegend(layerToAdd) {
    ...
    nameLayer = (sourceLayer === '1' || sourceLayer === '2') ? oCapaCurrent.capaxml : oCapaCurrent.url_fuente.replace('.zip', '');

    if (sourceLayer === '2' || sourceLayer === '5')
        urlLegendLayer = oCapaCurrent.url_fuente.indexOf('https') >= 0 ? `${oCapaCurrent.url_fuente.substr(0, oCapaCurrent.url_fuente.indexOf('?') + 1)}${URL_LENGEND_TYPE.replace('/wms?', '&')}${nameLayer}` : `${URL_PROXY}${oCapaCurrent.url_fuente.substr(0, oCapaCurrent.url_fuente.indexOf('?') + 1)}${URL_LENGEND_TYPE.replace('/wms?', '&')}${nameLayer}`
    else
        urlLegendLayer = URL_SERVICE_GEOSERVER + workspaceLayer + URL_LENGEND_TYPE + nameLayer + URL_LEGEND_PROP

    let htmlLegend = `<div id="legend_${oCapaCurrent.id}" class="section__legend"><label>${oCapaCurrent.capa}</label><br /><img class="legend__image" src="${urlLegendLayer}" alt="${oCapaCurrent.tematico_descripcion}: ${oCapaCurrent.capa}" ></img></div>`
...
async function onClickMap(evt) {
    ...
    map.eachLayer(function (lyr) {
        ...
        let oLayerInfo = { layer: lyr.options.layers, idfuente: lyr.options.idfuente, url: lyr.options.url_fuente, orden: lyr.options.orden, alias: lyr.options.alias, nombre: lyr.options.nombre };
    ...
    layersGetInfo.forEach(async function (item) {
        ...
        let dataResponse = await fetch(urlCurrentMD); //Deshabilitamos el puente de heroku
        // let dataResponse = await fetch(`${urlTemporal}`);
        let dataResponseText = await dataResponse.text();
        ...
        if (item.idfuente === 2 || item.idfuente === 5) {
            captionCurrent = `<span class="metadata__title">${item.nombre}</span>`
        }
        ...
        $buttonsMetadata = $contentData.querySelectorAll('button');
...
async function getFeatureInfoUrlStr(_evt, _lyr, _url, _alias) {
    ...
    let currentAlias = oLayerFeatureInfo.url_fuente.replace('.zip', '');
    let urlGetInfo = '';
